    ...
    T: Default,
{
    ...
    pub fn add(&mut self, value: T) {
        ...
        let mut add_idx = self.count;
        while add_idx > 1 {
            let parent_idx = self.parent_idx(add_idx);

            if (self.comparator)(&self.items[add_idx], &self.items[parent_idx]) {
                self.items.swap(add_idx, parent_idx);
                add_idx = parent_idx;
            } else {
                break;
            }
